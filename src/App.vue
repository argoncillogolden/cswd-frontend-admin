    ...
    <v-main>
      <template v-if="!isNotFoundPage">
        <v-app-bar v-if="showMenu" prominent>
          ...
          <img :src="logo" alt="DSWD Logo" height="50" class="ms-3" />
          <v-spacer></v-spacer>
          <v-btn icon>
            <!-- <v-badge 
              content="" 
        ...
        <v-navigation-drawer class="h-screen pa-3" v-model="drawer" v-if="showSidebar">
          <v-list density="compact" nav>
            ...
            <v-list-item prepend-icon="mdi-speedometer" @click="toDashboard" class="ps-3">Dashboard</v-list-item>
            <v-list-item prepend-icon="mdi-account-group-outline" @click="toAllBenef" class="ps-3">All
...
export default {
  ...
  mounted() {
    if (this.authStore.isAuthenticated && this.$route.name !== 'LoginPage') {
    ...
    updateFromBeneficiary() {
      echo.channel('received-cash-channel')
        .listen('CashReceived', (e) => {
          console.log(e);
          this.showSuccess('Confirmed cash received from ' + e.beneficiaryName, "success");
        });
    },
